...
*	主动发送消息
	...
	`params`: 
		*	`type`: `发送消息的类型, 由于网页版微信限制,目前只支持['text', 'image', 'video', 'file', ]四种类型`， 必填参数
		*	`file`: 文件对象。`当type为'image', 'video', 'file'中的一种时,file为必填参数
		*	`text`: 文本对象,一般为所要发送的的文本的内容。当type为`text`时,该参数必填
		*	`puid`: 消息接收对象的唯一身份标识,puid可以为好友,群组,公众号等任意一种。该参数为必填参数
	`return`:
		```python
			{
                   "id": 6920281997121075000,
...
#### 自定义
	...
	1.自定义的登录视图需要继承`wechat.LoginView`,并且需要制定`bot_class`
	2.自定义的`bot_class`需要继承默认的`DefaultBot`,并且需要制定定义消息处理的handle类列表`handler_classes`, 或者你可以重写`get_handler_classes`类方法。handler_classes里的每个handle类都会接收到消息。
	3.自定义的handle_class需要继承`BaseHandle`，想要处理不同的消息类型，只要在Handle_class里写上消息类型的小写的方法即可,如，想在一个Handle_class里面处理消息类型为`Text`的请求，如上`FirstHandle`即可。如果没有定义消息类型的方法，那么默认该条消息不会被处理,你也可以如上`SecondHandle`来修改默认行为。
